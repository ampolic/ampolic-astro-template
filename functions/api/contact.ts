...
export const onRequestPost: (ctx: {
  ...
  const fail = (msg: string, status = 400) =>
    wantsJson
      ...
      : new Response(
          `<h1>Could not send</h1><p>${msg}</p><a href="/contact">Back</a>`,
  ...
  if (!form.has("website") || !form.has("startedAt"))
  ...
  try {
    ...
    if (!sent.ok)
      return fail("We could not send your message. Please call us.", 502);
  } catch {
    return fail(
  ...
  return wantsJson
    ...
    : new Response(
        '<h1>Thank you</h1><p>We’ll be in touch shortly.</p><a href="/">Home</a>',
